...
{
  class MaxTakeOffWeightValidator : IServerValidator
  {
    private const string MaxTakeOffWeight = "MaxTakeOffWeight";
    ...
    public bool Validate(IList<FieldMetaData> fieldMetaDataList, MiscUatpErrorCodes MiscUatpErrorCodes, out DynamicValidationError validationError)
    {
      ...
      {
        var locationFields = fieldMetaData.SubFields.Where(subFields => subFields.FieldName == MaxTakeOffWeight).ToList();
        ...
        foreach (var metaData in locationFields)
        {
          foreach (var field in metaData.FieldValues)
          {
            if (field.AttributeValues.Count == 0)
            {
              ...
              {
                ErrorCode = MiscUatpErrorCodes.InvalidMaxTakeOffWeight,
                ErrorDescription = Messages.ResourceManager.GetString(MiscUatpErrorCodes.InvalidMaxTakeOffWeight)
              };
