...
{
  public class RouteDataValidator : IServerValidator
  {
    private const string Routedata = "RouteData";
    ...
    public bool Validate(IList<FieldMetaData> fieldMetaDataList, MiscUatpErrorCodes MiscUatpErrorCodes, out DynamicValidationError validationError)
    {
      ...
      {
        var locationFields = fieldMetaData.SubFields.Where(subFields => subFields.FieldName == Routedata).ToList();
        ...
        foreach (var metaData in locationFields)
        {
          foreach (var field in metaData.FieldValues)
          {
            if (field.AttributeValues.Count == 0)
            {
              ...
              {
                ErrorCode = MiscUatpErrorCodes.InvalidRouteDataType,
                ErrorDescription = Messages.ResourceManager.GetString(MiscUatpErrorCodes.InvalidRouteDataType)
              };
