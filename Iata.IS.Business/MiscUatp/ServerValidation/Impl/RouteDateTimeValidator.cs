...
{
  public class RouteDateTimeValidator : IServerValidator
  {
    private const string Routedatetime = "RouteDateTime";
    ...
    public bool Validate(IList<FieldMetaData> fieldMetaDataList, MiscUatpErrorCodes MiscUatpErrorCodes, out DynamicValidationError validationError)
    {
      ...
      {
        var locationFields = fieldMetaData.SubFields.Where(subFields => subFields.FieldName == Routedatetime).ToList();
        ...
        foreach (var metaData in locationFields)
        {
          foreach (var field in metaData.FieldValues)
          {
            if (field.AttributeValues.Count == 0)
            {
              ...
              {
                ErrorCode = MiscUatpErrorCodes.InvalidRouteDateTimeType,
                ErrorDescription = Messages.ResourceManager.GetString(MiscUatpErrorCodes.InvalidRouteDateTimeType)
              };
