...
{
  public class PartialPaymentsValidator : IServerValidator
  {
    private const string ReferenceNumber = "PartialPayments";
    ...
    public bool Validate(IList<FieldMetaData> fieldMetaDataList, MiscUatpErrorCodes MiscUatpErrorCodes, out DynamicValidationError validationError)
    {
      ...
      foreach (var fieldMetaData in fieldMetaDataList)
      {
        ...
        foreach (var metaData in locationFields)
        {
          ...
          {
            if (field.AttributeValues.Count != 2)
            {
              validationError = new DynamicValidationError
              {
                ErrorCode = MiscUatpErrorCodes.InvalidPartialPaymentsAttribute,
                ErrorDescription = Messages.ResourceManager.GetString(MiscUatpErrorCodes.InvalidPartialPaymentsAttribute)
              };
