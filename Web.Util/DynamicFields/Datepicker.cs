...
namespace Iata.IS.Web.Util.DynamicFields
{
  ...
  {
    public Datepicker(HtmlHelper html)
      : base(html)
    {
    ...
    protected new string GetValue(FieldMetaData field, string controlValue = null)
    {
      try
      {
        ...
        else if(field.FieldValues[0] != null)
        {
        ...
        {
          DateTime dtValue;
          bool isParsed = DateTime.TryParseExact(selectedValue, FormatConstants.DateFormat, CultureInfo.InvariantCulture, DateTimeStyles.None, out dtValue);
          if (isParsed)
            return dtValue.ToString(FormatConstants.DateFormat);
