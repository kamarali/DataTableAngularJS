...
{
  public class RMAwbModelBinder : DefaultModelBinder
  {
    ...
    public override object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
    {
      ...
      var awbRecord = model as RMAwb;

      if (awbRecord != null)
      {
        // Convert fields to uppercase.
        awbRecord.ConsignmentOriginId = !string.IsNullOrEmpty(awbRecord.ConsignmentOriginId) ? awbRecord.ConsignmentOriginId.ToUpper() : null;
        awbRecord.ConsignmentDestinationId = !string.IsNullOrEmpty(awbRecord.ConsignmentDestinationId) ? awbRecord.ConsignmentDestinationId.ToUpper() : null;

        awbRecord.CarriageFromId = !string.IsNullOrEmpty(awbRecord.CarriageFromId) ? awbRecord.CarriageFromId.ToUpper() : null;
        awbRecord.CarriageToId = !string.IsNullOrEmpty(awbRecord.CarriageToId) ? awbRecord.CarriageToId.ToUpper() : null;

        awbRecord.PartShipmentIndicator = !string.IsNullOrEmpty(awbRecord.PartShipmentIndicator) ? awbRecord.PartShipmentIndicator.ToUpper() : null;

        awbRecord.CurrencyAdjustmentIndicator = !string.IsNullOrEmpty(awbRecord.CurrencyAdjustmentIndicator) ? awbRecord.CurrencyAdjustmentIndicator.ToUpper() : null;
        ...
        foreach (string otherchar in otherchargeIds)
        {
          ...
          if (string.IsNullOrEmpty(id))
            continue;
          awbRecord.OtherCharge.Add(new RMAwbOtherCharge()
          {
        ...
        foreach (string code in vatIds)
        {
          ...
          if (string.IsNullOrEmpty(id))
            continue;
          awbRecord.AwbVat.Add(new RMAwbVat()
          {
        ...
        GetProrateLadderFields(awbRecord, form);
        ...
        foreach (string code in attachmentIds)
        {
          ...
          if (string.IsNullOrEmpty(id))
            continue;
          awbRecord.Attachments.Add(new RMAwbAttachment()
          {
        ...
        model = ModelBinderHelper.SplitAwbSerialNumber(awbRecord, awbSerialNumberCheckDigit) as RMAwb;
      }

      return model;
    }

    private static void GetProrateLadderFields(RMAwb awbRecord, NameValueCollection form)
    {
      ...
      foreach (string prorateLadder in prorateLadderIds)
      {
        ...
        if (string.IsNullOrEmpty(id))
          continue;
        awbRecord.ProrateLadder.Add(new RMAwbProrateLadderDetail()
        {
