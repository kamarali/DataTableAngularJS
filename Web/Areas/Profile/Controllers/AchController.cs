...
{
  public class AchController : ISController
  {
    //
    // GET: /Profile/Ach/
    ...
    private const int AchZoneId = 3;

    public AchController(IMemberManager memberManager, IBlockingRulesManager blockingRulesManager)
    {
   ...
   [ISAuthorize(Business.Security.Permissions.AchOps.ManageBlocksAccess)]
    public ActionResult AchBlockingRules()
    {
      const string memberId = "";
      const string ruleName = "";
      const string description = "";
      ViewData["clearingHouse"] = "ACH";
      var blockingRulesGrid = new BlockingRules("BlockingRuleGrid", Url.Action("BlockingRulesGridData", "Ach", new { memberId, ruleName, description }));
      ViewData["BlockingRuleGrid"] = blockingRulesGrid.Instance;
      return View();
    }

   [ISAuthorize(Business.Security.Permissions.AchOps.ManageBlocksAccess)]
   public ActionResult GetAchBlockingRules()
   {
     ...
     var csvFileName = Path.Combine(csvReportTempPath,
                                    string.Format("ACH Blocking Rules Download-{0}{1}{2}-{3}{4}{5}.csv",
     ...
     // Get csv report data.
     var achBlockingRules = _blockingRulesManager.GetBlokingRulesForClearingHouse("ACH") ??
                            new List<DownloadBlockingRules>();

     var csvGenerator = new CsvGenerator();

     // Generate Csv Report.
     csvGenerator.GenerateCSV(achBlockingRules, csvFileName);
    ...
    public ActionResult AchBlockingRules(BlockingRule blockingRule)
    {
      const string memberId = "";
      ...
      {
        blockingRulesGrid = new BlockingRules("BlockingRuleGrid", Url.Action("BlockingRulesGridData", "Ach",
                                                                                new
                                                                                  {
    ...
    public JsonResult BlockingRulesGridData(string memberId, string ruleName, string description)
    {
      ...
      var blockingRules = _blockingRulesManager.GetBlockingRuleList(memberId, ruleName, description, "ACH");
      try
      {
    ...
    public ActionResult BlockingRuleDetails(BlockingRule blockingRule, IList<BlockMember> items, IList<BlockGroup> blockGroupItems, IList<BlockGroupException> blockGroupExceptionItems)
    {
      ...
      try
      {
        ...
        if (items != null)
        {
          ...
          {
            // if member id is 0 means it has been deleted client side and should not be 
            // added in database.
            if (item.IsDeleted == true)
              continue;

            // storing the value of Pax,Cargo,Uatp,Misc checkboxes value in MemberText property at 
            // client side as these checkboxes is disable so their values always be false irrespective whether they are checked or not.
            string[] memberType = item.MemberText.Split(',');
        ...
        if (blockGroupItems != null)
        {
            ...
            foreach (var groupItem in blockGroupItems)
            {
                ...
                blockGroup.ZoneTypeId = AchZoneId;
    ...
    public ActionResult BlocksByGroup(int blockingRuleId)
    {
        ...
        blocksgrid.Instance.Columns[2].HeaderText = "Zone";
    ...
    public JsonResult BlockesByGroupBlocksGridData(int blockingRuleId)
    {
        ...
        gridModel.Instance.Columns[2].HeaderText =  "Zone";
    ...
    public string GetBlockedCreditorsId(int blockingRuleId)
    {
        ...
        {
            memberIdString += "," + blockGroup.MemberId.ToString();
        }// end foreach()
