    ...
    if (exceptionRowCount > 0 || isEditExceptionFlag) {
      ...
      if (ids == null) {
        ...
        if (isTempRowId > 0) {
          ...
          jQuery("#BlocksByGroupExceptionsGrid").jqGrid('setGridParam', { url: '<%: Url.Action("BlockesByGroupExceptionsGridData", "ACH", new { area = "Profile"})%>?groupId=' + ids, page: 1 });
          jQuery("#BlocksByGroupExceptionsGrid").trigger('reloadGrid');
        }
      } // end else
    } // end else
...
<div style="float: left;">
  <h2>
    Blocks By Group</h2>
  <div>
    <%Html.RenderPartial("~/Areas/Profile/Views/ACH/BlocksByGroupBlocksControl.ascx", ViewData["BlocksByGroupBlocksGrid"]); %>
  </div>
...
<div style="float: left; padding-left: 20px;">
  <h2>
    Exceptions</h2>
  <div>
    <%Html.RenderPartial("~/Areas/Profile/Views/ACH/BlocksByGroupExceptionsControl.ascx", ViewData["BlocksByGroupExceptionsGrid"]); %>
  </div>
  <div class="buttonContainer">
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
      ...
      $AddBlockGroupExceptiondialog = $('<div></div>')
		...
		.dialog({
            ...
            open: function (event, ui) { registerAutocomplete('ExceptionMemberText', 'ExceptionMemberId', '<%:Url.Action("GetMemberListForIchOrAch", "Data", new { area = "" , menuType="onlyAch"})%>', 0, true, null, '', '', "#hdnACHZoneId"); }
		});
  });
  ...
  function ShowAddBlockGroupExceptiondialog() {
    ...
    if (groupId == null) {
      ...
      var oldZoneId = $("#hdnACHZoneId").val();
