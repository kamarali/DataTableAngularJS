...
<h1>
        Manage Blocks</h1>
    <h2>
        ACH Blocking Rules</h2>
    <div>
        Last Updated:
        <%: string.Format("{0}", DateTime.UtcNow.AddDays(-1).AddMinutes(-34)) %>
        by ICH2091</div>
        <% using (Html.BeginForm("AchBlockingRules", "Ach", FormMethod.Post))
   {%>
   <%: Html.AntiForgeryToken() %>
    <div>
      ...
      <div>
    <input type="button" class="primaryButton" value="Download All Block Information" onclick="javascript:location.href ='<%: Url.Action("GetAchBlockingRules", "Ach") %>'" />
  </div>
</asp:Content>
