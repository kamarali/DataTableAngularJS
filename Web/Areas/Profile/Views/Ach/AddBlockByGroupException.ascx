...
<script type="text/javascript">
  $(document).ready(function () {
    // CMP#596: CIT9445:12 chars of accounting code is not displayed in "Exceptions" section "Blocked By Group" of ACH/ICH Blocking Rule.
    // registerAutocomplete('ExceptionMemberText', 'ExceptionMemberId', '<%:Url.Action("GetMemberListForIchOrAch", "Data", new { area = "" ,menuType="onlyAch"})%>', 0, true, null, '', '', "#hdnACHZoneId");
...
<script type="text/javascript">
  ...
  function addBlockbyGroupException() {
    ...
    if (ownerMember == exceptionMember) {
      ...
      var isMemberCodeAdded = AddBlockMemberException('<%:Url.Action("GetMemberDetails", "Ach", new { area = "Profile"}) %>');
  ...
  function DeleteExceptionRowFromDatabase(id) {
    ...
    $('#DeletedExceptionRowString').val($('#DeletedExceptionRowString').val() + ',' + groupId + '|' + id);
      ...
      // Hide the row
    var rowId = jQuery('#BlocksByGroupExceptionsGrid').getGridParam('selrow');
      $('#BlocksByGroupExceptionsGrid tr[id=' + rowId + ']').hide();
