...
<script type="text/javascript">
  ...
  $(document).ready(function () {
      $('#ichMemberText').focus();
    // Set DeletedExceptionRowString hidden field to "" 
    $("#DeletedExceptionRowString").val('');
  ...
  $(function () {
    ...
    //IATA SIS-P4-Third party : Jquery And Jquery UI 1.12.3 Upgradation
      ...
      $('.achBlkcred-dialog').remove();
      $('.achBlkDebt-dialog').remove();
    });
  });

  function ICHMemberText_AutoCompleteValueChange(selectedId) {
  ...
  function SaveIchBlockedMemberDetails() {
    ...
    var ownerMemberId = $("#MemberId").val();
    ...
    if ($('#IsInEditMode').val()) {
      ...
      $.ajax({
        type: "POST",
        url: '<%: Url.Action("UpdateBlockingMember", "Ich", new { area = "Profile"}) %>',
        dataType: "json",
      ...
      $.ajax({
        type: "POST",
        url: '<%: Url.Action("UpdateBlockGroup", "Ich", new { area = "Profile"}) %>',
        dataType: "json",
...
<% using (Html.BeginForm("BlockingRuleDetails", "Ich", FormMethod.Post, new { id = "BlockingRule" }))
   {%>
...
<div>
  ...
  <div class="solidBox dataEntry">
    <div class="fieldContainer horizontalFlow">
      <div>
        <div style=" width:250px;">
          ...
          <%: Html.TextBoxFor(model => model.MemberText, new { @id = "ichMemberText", @disabled = true, @class = "autocComplete textboxWidth" })%>
        ...
        <!-- To Add Space between the controls -->
        <div>
        </div>
        <div style=" width:250px;">
          <label>
    ...
    <div id="tabs" class="ui-tabs-hide">
      <ul>
        <li>
          <%:Html.ActionLink("Blocked Creditors", "BlockedCreditors", "Ich",
                  new { area = "Profile", blockingRuleId = Model != null ? Model.Id : 0, memberId = Model != null ? Model.MemberId : 0 }, null)%>
        </li>
        <li>
          <%:Html.ActionLink("Blocked Debtors", "BlockedDebitors", "Ich",
                  new { area = "Profile", blockingRuleId = Model != null ? Model.Id : 0, memberId = Model != null ? Model.MemberId : 0 }, null)%>
        </li>
        <li>
          <%:Html.ActionLink("Blocks By Group", "BlocksByGroup", "Ich",
  ...
  <div>
    <div class="buttonContainer">
      <input type="submit" class="primaryButton" value="Save" id="btnSave" name="btnSave" onclick="javascript:return SaveIchBlockedMemberDetails();" />
      <%: Html.LinkButton("Back", Url.Action("IchBlockingRules", "Ich"))%>
    </div>
  </div>
</div>
<%
