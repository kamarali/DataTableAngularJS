    ...
    <div class="fieldContainer horizontalFlow">
      <div>
        <div>
          ...
          <%:Html.IchZoneDropdownListFor(model => model.ZoneTypeId, new Dictionary<string,object>{ { "id", "ZoneTypeId" }})%>
        </div>
        <div>
          <label>
    ...
    jQtable.each(function () {
      ...
      if (zoneId != "" && blockedAgainst != "" && !areCheckBoxesChecked) {
        ...
        if (BlockByGroupZoneIdString == "") {
          $.ajax({
            type: "GET",
            url: '<%: Url.Action("GetBlockByGroupsZoneId", "Ich", new { area = "Profile"}) %>',
        ...
        for (var i = 0; i < zoneIdArray.length; i++) {
          if (zoneIdArray[i] == zoneId + blockedAgainst) {
            ...
            $("#ZoneTypeId").val($("#ZoneTypeId option:first").val()); 
