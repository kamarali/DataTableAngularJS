...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
	SIS :: UATP :: <%:ViewData[ViewDataConstants.BillingType]%> :: <%:ViewData[ViewDataConstants.PageMode] %> Line Item Detail
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <h1>
    <%:ViewData[ViewDataConstants.PageMode] %> Line Item Detail</h1>
  <div>
  ...
  <% using (Html.BeginForm("LineItemDetailEdit", "UatpCreditNote", FormMethod.Post, new { id = "LineItemDetailForm", @class = "validCharacters" }))
     {  %>
...
<div class="buttonContainer">
    ...
    <% if (Model.LineItemDetail.Id != Model.LineItemDetail.NavigationDetails.FirstId)
       {%>
    <input class="secondaryButton" value="First" onclick="javascript:location.href='<%: Url.Action(actionToRedirect, "UatpPayCreditNote", new { invoiceId = Model.LineItemDetail.LineItem.InvoiceId, lineItemId = Model.LineItemDetail.LineItemId, lineItemDetailId = Model.LineItemDetail.NavigationDetails.FirstId }) %>'"
      type="button" />
    <input class="secondaryButton" value="Previous" onclick="javascript:location.href='<%: Url.Action(actionToRedirect, "UatpPayCreditNote", new { invoiceId = Model.LineItemDetail.LineItem.InvoiceId, lineItemId = Model.LineItemDetail.LineItemId, lineItemDetailId = Model.LineItemDetail.NavigationDetails.PreviousId }) %>'"
      type="button" />
    <%}
       else
       {%>
    ...
    <% if (Model.LineItemDetail.Id != Model.LineItemDetail.NavigationDetails.LastId)
       {%>
    <input class="secondaryButton" value="Next" onclick="javascript:location.href='<%: Url.Action(actionToRedirect, "UatpPayCreditNote", new { invoiceId = Model.LineItemDetail.LineItem.InvoiceId, lineItemId = Model.LineItemDetail.LineItemId, lineItemDetailId = Model.LineItemDetail.NavigationDetails.NextId }) %>'"
      type="button" />
    <input class="secondaryButton" value="Last" onclick="javascript:location.href='<%: Url.Action(actionToRedirect, "UatpPayCreditNote", new { invoiceId = Model.LineItemDetail.LineItem.InvoiceId, lineItemId = Model.LineItemDetail.LineItemId, lineItemDetailId = Model.LineItemDetail.NavigationDetails.LastId }) %>'"
      type="button" />
    <%}
       else
       {%>
   ...
   <div class="buttonContainer">
    <input class="primaryButton ignoredirty" type="submit" value="Save And Add" id="Save" onclick="javascript:return changeAction('<%: Url.Action("LineItemDetailEdit","UatpCreditNote", new { lineItemDetailId = Model.LineItemDetail.Id.ToString() }) %>'); return false;" />
    <input class="primaryButton ignoredirty" type="submit" value="Save And Duplicate" id="SaveDuplicate" onclick="javascript:return changeAction('<%: Url.Action("LineItemDetailClone","UatpCreditNote", new { lineItemDetailId = Model.LineItemDetail.Id.ToString() })%>')" />
    <input class="primaryButton ignoredirty" type="submit" value="Save And Back" id="SaveReturn" onclick="javascript:return changeAction('<%: Url.Action("LineItemDetailEditAndReturn","UatpCreditNote", new { lineItemDetailId = Model.LineItemDetail.Id.ToString() }) %>');" />
    <% if (ViewData[ViewDataConstants.PageMode] != null && ViewData[ViewDataConstants.PageMode].ToString() == PageMode.View)
    { %>
      <%: Html.LinkButton("Back", Url.Action("LineItemView", "UatpPayCreditNote"))%>
    <%} else {%>
