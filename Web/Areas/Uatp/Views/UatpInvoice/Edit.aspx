...
<asp:Content ID="Content1" ContentPlaceHolderID="Script" runat="server">
  ...
  <%
   ...
   isOnEditInvoice = true;
    ...
    InitializeAttachmentGrid(<%=new JavaScriptSerializer().Serialize(Model.Attachments)%>, '<%:new FileAttachmentHelper().GetValidFileExtention(Model.BilledMemberId, Model.BillingCategory)%>', '<%:Url.Action("DeleteAttachment", "UatpInvoice")%>','<%:Url.Action("InvoiceAttachmentDownload", "UatpInvoice")%>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
  ...
  while (part_num < selectedLineItemIds.length)
 {
  ...
  part_num++;
      ...
      {
%>CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "p", '<%:Url.Action("RejectInvoice", "UatpInvoice")%>');
     <%
      }
      else if (TempData["Reject"] != null && Convert.ToBoolean(TempData["Reject"])) // from billing history
      {%>
  CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "bh", '<%:Url.Action("RejectInvoice", "UatpInvoice")%>');
  <%
      }%>
    ...
    <%
        ...
        {%>
    <% =ScriptHelper.GenerateGridEditDeleteScript(Url, ControlIdConstants.LineItemGridId, Url.Action("LineItemEdit", "UatpInvoice"), Url.Action("LineItemDelete", "UatpInvoice"))%>
    <%
    }
    else
    {%>
