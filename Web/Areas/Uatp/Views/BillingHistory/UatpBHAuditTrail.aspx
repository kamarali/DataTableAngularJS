...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  SIS :: Uatp :: Billing History Audit Trail
</asp:Content>
    ...
    var rejectionInv = new MiscUatpInvoice();
      ...
      else
      {
        rejectionInv = Model.RejectionInvoiceList.OrderByDescending(invoice => invoice.RejectionStage).ToList()[0];
			...
			{
				if ((correspondence.CorrespondenceSubStatus != CorrespondenceSubStatus.Saved && correspondence.CorrespondenceSubStatus != CorrespondenceSubStatus.ReadyForSubmit && correspondence.CorrespondenceSubStatus != CorrespondenceSubStatus.DueToExpiry) || ((correspondence.CorrespondenceSubStatus == CorrespondenceSubStatus.Saved || correspondence.CorrespondenceSubStatus == CorrespondenceSubStatus.ReadyForSubmit ) && correspondence.FromMemberId == SessionUtil.MemberId))
				{
%><h2>
Stage
			 ...
			 foreach (var rejectionInvoice in Model.RejectionInvoiceList.OrderByDescending(invoice => invoice.RejectionStage).ToList())
{
  ...
  <div class="buttonContainer">
    ...
    <%--<input type="button" class="secondaryButton" value="Generate PDF" onclick="javascript:location.href ='<%: Url.Action("GenerateBillingHistoryAuditTrailPdfForUatp", "BillingHistory", new {invoiceId = ViewData["invoiceId"].ToString(), areaName = "Uatp"}) %>'" />--%>
    <input type="button" class="secondaryButton" value="Generate PDF" onclick="DownloadFile('<%: Url.Action("GenerateBillingHistoryAuditTrailPdfForUatp", "BillingHistory", new {invoiceId = ViewData["invoiceId"].ToString(), areaName = "Uatp"}) %>','<%: Url.Action("EnqueBillingHistoryAuditTrailDownload", "BillingHistory", new {invoiceId = ViewData["invoiceId"].ToString(), areaName = "Uatp", billingCategory = BillingCategoryType.Uatp}) %>')" />
