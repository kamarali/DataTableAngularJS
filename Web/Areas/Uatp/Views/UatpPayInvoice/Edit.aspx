<%@ Page Inherits="System.Web.Mvc.ViewPage<Iata.IS.Model.MiscUatp.MiscUatpInvoice>"
...
<asp:Content ID="Content1" ContentPlaceHolderID="Script" runat="server">
  ...
  <%
   ...
   isOnEditInvoice = true;
    ...
    InitiliazeCorrespondenceFields('<%:Model.Id%>', true); 
      ...
      '<%:Url.Action("GetRejectionInvoiceDetails", "UatpPayInvoice", new { area = "Uatp" })%>', 
      '<%:Url.Action("GetCorrespondenceInvoiceDetails", "UatpPayInvoice", new { area = "Uatp" })%>',
      '', 
    ...
    InitializeAttachmentGrid(<%=new JavaScriptSerializer().Serialize(Model.Attachments)%>, '<%:new FileAttachmentHelper().GetValidFileExtention(Model.BilledMemberId, Model.BillingCategory)%>', '<%:Url.Action("DeleteAttachment", "UatpInvoice")%>','<%:Url.Action("InvoiceAttachmentDownload", "UatpPayInvoice")%>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
  ...
  while (part_num < selectedLineItemIds.length)
 {
  ...
  part_num++;
      ...
      {
%>CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "p", '<%:Url.Action("RejectInvoice", "UatpPayInvoice")%>');
     <%
      }
      else if (TempData["Reject"] != null && Convert.ToBoolean(TempData["Reject"])) // from billing history
      {%>
  CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "bh", '<%:Url.Action("RejectInvoice", "UatpPayInvoice")%>');
  <%
      }%>
    ...
    <%
        ...
        {%>
    <% =ScriptHelper.GenerateGridEditDeleteScript(Url, ControlIdConstants.LineItemGridId, Url.Action("LineItemEdit", "UatpInvoice"), "LineItemDelete")%>
    <%
    }
    else
    {%>
...
<asp:Content ContentPlaceHolderID="MainContent" runat="server">
    <h1>
        ...
        UATP Invoice</h1>
    <%
        using (Html.BeginForm("Edit", "UatpInvoice", FormMethod.Post, new { id = "InvoiceForm", @class = "validCharacters" }))
        {%>
    ...
    <div class="buttonContainer">
        ...
        <input class="secondaryButton" type="submit" value="Add Line Item" id="AddLineItem"
            onclick="javascript:location.href='<%:Url.Action("LineItemCreate")%>'; return false;" />
    ...
    <div class="buttonContainer">
        <%
            ...
            {
                // SCP#419602: SRM: CGO/MISC/UATP and Billing history permissions [UATP Payables]
        ...
        <%
            if (Model.InvoiceStatus == InvoiceStatusType.Open && Html.IsAuthorized(Iata.IS.Business.Security.Permissions.UATP.Receivables.Invoice.Validate)) // || Model.InvoiceStatus == InvoiceStatusType.ReadyForValidation)
            {
                using (Html.BeginForm("ValidateInvoice", "UatpInvoice", FormMethod.Post))
                {%>
                <%: Html.AntiForgeryToken() %>
        ...
        <%
            ...
            if (Model.InvoiceStatus == InvoiceStatusType.ReadyForSubmission && Html.IsAuthorized(Iata.IS.Business.Security.Permissions.UATP.Receivables.Invoice.Submit))
      {
          using (Html.BeginForm("Submit", "UatpInvoice", FormMethod.Post))
          {
        %>
        <%: Html.AntiForgeryToken() %>
      ...
      {%>
        ...
        %>
        <%
            if (SessionUtil.SearchType == "UatpBillingHistory")
            {
        %>
