...
<asp:Content ID="Content1" ContentPlaceHolderID="Script" runat="server">
  ...
  <%
    ...
    InitializeBilateralSMIs(<%: Convert.ToInt32(SMI.Bilateral) %>  ,'<%: ViewData[ViewDataConstants.BilateralSMIs] %>');
  	InitialiseInvoiceHeader('<%:Url.Action("GetMiscUatpBilledMemberLocationList", "Data", new { area = "" })%>', '<%:Url.Action("GetExchangeRate", "Data", new { area = "" })%>', '<%:Url.Action("GetRejectionInvoiceDetails", "UatpCreditNote", new { area = "Uatp" })%>', '<%:Url.Action("GetCorrespondenceInvoiceDetails", "UatpInvoice", new { area = "Uatp" })%>','', '<%:Url.Action("GetDefaultSettlementMethod", "Data", new { area = "" })%>', '<%:Url.Action("GetLocationDetails", "Data", new { area = "" })%>','<%:SessionUtil.MemberId%>', '<%:Model.BillingCategoryId%>',
    ...
    InitializeAttachmentGrid(<%=new JavaScriptSerializer().Serialize(Model.Attachments)%>, '<%:new FileAttachmentHelper().GetValidFileExtention(Model.BilledMemberId, Model.BillingCategory)%>', '<%:Url.Action("DeleteAttachment", "UatpCreditNote")%>','<%:Url.Action("InvoiceAttachmentDownload", "UatpCreditNote")%>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
  ...
  while (part_num < selectedLineItemIds.length)
 {
  ...
  part_num++;
      ...
      {
%>CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "p", '<%:Url.Action("RejectInvoice", "UatpCreditNote")%>');
     <%
      }
      else if (TempData["Reject"] != null && Convert.ToBoolean(TempData["Reject"])) // from billing history
      {%>
  CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "bh", '<%:Url.Action("RejectInvoice", "UatpCreditNote")%>');
  <%
      }%>
  ...
  <%
    ...
    {%>
  <%: ScriptHelper.GenerateGridEditDeleteScript(Url, ControlIdConstants.LineItemGridId, Url.Action("LineItemEdit", "UatpCreditNote"), Url.Action("LineItemDelete", "UatpCreditNote"))%>
  <%
    }
    else
    {%>
...
<asp:Content ContentPlaceHolderID="MainContent" runat="server">
  <h1>
    ...
    UATP Credit Note</h1>
  <%
    using (Html.BeginForm("Edit", "UatpCreditNote", FormMethod.Post, new { id = "InvoiceForm", @class = "validCharacters" }))
    {%>
...
{%>
  ...
  <div>
   ...
   <div class="buttonContainer">
    ...
    %>
    <%
       if (Model.InvoiceStatus == InvoiceStatusType.Open && Html.IsAuthorized(Iata.IS.Business.Security.Permissions.UATP.Receivables.CreditNote.Validate)) // || Model.InvoiceStatus == InvoiceStatusType.ReadyForValidation)
       {
         using (Html.BeginForm("ValidateInvoice", "UatpCreditNote", FormMethod.Post))
         {%>
         <%: Html.AntiForgeryToken() %>
    ...
    <%
      ...
      if (Model.InvoiceStatus == InvoiceStatusType.ReadyForSubmission && Html.IsAuthorized(Iata.IS.Business.Security.Permissions.UATP.Receivables.CreditNote.Submit))
      {
        using (Html.BeginForm("Submit", "UatpCreditNote", FormMethod.Post))
        {
    %>
    <%: Html.AntiForgeryToken() %>
