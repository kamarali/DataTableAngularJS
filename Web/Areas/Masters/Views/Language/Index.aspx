...
<% using (Html.BeginForm("Index", "Language", FormMethod.Post))
    ...
    <div>
        <% Html.RenderPartial("~/Areas/Masters/Views/Shared/SearchLanguage.ascx"); %>
    </div>
    <div class="buttonContainer">
    <%if (Html.IsAuthorized(Iata.IS.Business.Security.Permissions.Masters.Masters.LanguageSetupEditOrDelete))
{%>
        <input type="button" class="primaryButton" value="Add" id="btnAdd" name="Add" onclick="javascript:location.href = '<%:Url.Action("Create", "Language")%>'" />
        <%
}%>

<%if (Html.IsAuthorized(Iata.IS.Business.Security.Permissions.Masters.Masters.LanguageSetupQuery)) 
   {%>
    ...
    <%Html.RenderPartial("~/Areas/Masters/Views/Shared/SearchLanguageGrid.ascx", ViewData["LanguageGrid"]); %>
