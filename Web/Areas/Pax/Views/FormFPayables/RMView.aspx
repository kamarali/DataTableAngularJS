...
<asp:Content ID="Content3" ContentPlaceHolderID="Script" runat="server">
  ...
  <script type="text/javascript">
    $(document).ready(function () {
      ...
      InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.Invoice.BilledMemberId, BillingCategoryType.Pax) %>', '<%: Url.Action("RMFormFXFAttachmentDownload","FormFPayables", new { invoiceId = Model.InvoiceId } ) %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
    });
  </script>
</asp:Content>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div>    
    ...
    <input class="secondaryButton" type="button" value="Back" onclick="javascript:location.href='<%: Url.Action("View", "FormFPayables", new { invoiceId = Model.Invoice.Id.Value() } ) %>'" />
    <%} %>
    </div>
  </div>
  <h2>
