...
<asp:Content ID="Content3" ContentPlaceHolderID="Script" runat="server">
  ...
  <script type="text/javascript">
    ...
    InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.Invoice.BilledMemberId,BillingCategoryType.Pax) %>', '<%: Url.Action("RejectionMemoAttachmentDownload","InvoicePayables") %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");      
...
%>
<input class="secondaryButton" type="button" value="Back" onclick="javascript:location.href = '<%:Url.Action("RMList","InvoicePayables",new{invoiceId = Model.InvoiceId})%>';" />
