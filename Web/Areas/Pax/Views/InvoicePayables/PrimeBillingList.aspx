  ...
  <div class="buttonContainer">
    ...
    <%
      ...
      else
      {
        ...
        {%>
    <input class="primaryButton" type="button" value="Reject" onclick="javascript:InitiateRejForSpecificTrans('<%: ViewDataConstants.PrimeBillingGrid %>', 'PC', '<%: Model.Id %>', '<%:Url.Action("InitiateRejection", "InvoicePayables", new { area = "Pax" })%>','<%:Url.Action("InitiateDuplicateRejection", "InvoicePayables", new { area = "Pax" })%>', <%:Model.BillingCode %>, <%:Model.BillingYear %>,<%:Model.BillingMonth %>,<%:Model.BillingPeriod %>,<%:Model.SettlementMethodId %>);" id ="RejectButton"/>
    <%}%>
    <input class="secondaryButton" type="button" value="Back" onclick="javascript:location.href = '<%:Url.Action("View","InvoicePayables",new{invoiceId = Model.Id.Value()})%>';" />
    <%
      }%>
  </div>
