...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div class="buttonContainer">
    ...
    <div id="divTransactions">
        ...
        {%>
    <input class="secondaryButton" type="button" value="Reject" onclick="javascript:InitiateRejForSpecificTrans('<%: ControlIdConstants.FormDGridId %>', 'FD', '<%: Model.Id %>', '<%:Url.Action("InitiateRejection", "FormDEPayables", new { area = "Pax" })%>','<%:Url.Action("InitiateDuplicateRejection", "FormDEPayables", new { area = "Pax" })%>', <%:Model.BillingCode %>, <%:Model.BillingYear %>,<%:Model.BillingMonth %>,<%:Model.BillingPeriod %>,<%:Model.SettlementMethodId %>);" id ="RejectButton"/>
    <%}%>
      <% if (ViewData[ViewDataConstants.PageMode].ToString() == PageMode.View)
         {
             using (Html.BeginForm("FormEView", "FormDEPayables", FormMethod.Get))
           {%>
      ...
      <% if (ViewData[ViewDataConstants.PageMode].ToString() == PageMode.View)
         {
             using (Html.BeginForm("ProvisionalInvoiceView", "FormDEPayables", FormMethod.Get))
           {%>
      ...
      <% using (Html.BeginForm(((ViewData[ViewDataConstants.PageMode].ToString() == PageMode.View) ? "VatView" : "Vat"), "FormDEPayables", FormMethod.Get))
         { %>
