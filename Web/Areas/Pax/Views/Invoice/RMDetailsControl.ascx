  ...
  <div class="fieldContainer horizontalFlow">
    ...
    <div class="bottomLine">
      ...
      <div>
        ...
        <%:Html.TextBoxFor(m => m.FimBMCMNumber, new { @maxLength = 11, @class = "alphaNumeric" })%>
        <%:Html.HiddenFor(m => m.FimBMCMNumber)%>
      </div>
      <div>
        <label>
          FIM Coupon Number:</label>
        <%:Html.TextBoxFor(m => m.FimCouponNumber, new { @class = "integer", @maxLength = 2 })%>
        <%:Html.HiddenFor(m => m.FimCouponNumber)%>
      </div>
      <%
...
%>
    ...
    <div class="bottomLine">
      ...
      <%--CMP #671: Validation of PAX CGO Stage 2 & 3 Rejection Memo Reason Text. Desc: Apply  Text Area Trim CSS Class to remove excessive \n on every load.--%>
      <%:Html.TextAreaFor(m => m.ReasonRemarks, 10, 80, new { maxlength = "4000", @class = "notValidCharsTextarea textAreaTrimText" })%>
    </div>
    <div class="bottomLine">
