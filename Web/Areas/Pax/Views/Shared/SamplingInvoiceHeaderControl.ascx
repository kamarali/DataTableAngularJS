  ...
  <div class="fieldContainer horizontalFlow">
    <div>
      ...
      <div>
        ...
        <%:Html.ProvisionalBillingMonthFormDEDropdownList(ControlIdConstants.ProvisionalBillingMonthFormDEDropdown, Model.ProvisionalBillingMonth, Model.ProvisionalBillingYear)%>
      </div>
      <div>
        <%: ScriptHelper.GenerateDialogueHtml("Billing Member’s Location ID:", "Billing Member Reference Data", "BillingMemberReference", 535, 700)%>
    ...
    <div>
      ...
      <div>
        ...
        <%:Html.TextBox("InvoiceDate", Model.InvoiceDate.ToString(FormatConstants.DateFormat), new { @readonly = true })%>
      </div>
      <div>
        <label for="ListingCurrency">
          <span>*</span> Currency of Listing/Evaluation:</label>
        <%:Html.CurrencyDropdownList(ControlIdConstants.ListingCurrencyId, Model.ListingCurrencyId.ToString())%>
      </div>
      <div>
        <label>
    ...
    <div>
      ...
      <div class="Hidden">
        ...
        <%:Html.HiddenFor(invoice => invoice.IsFormABViaIS)%>
        <%:Html.HiddenFor(invoice => invoice.IsFormCViaIS)%>
      </div>
    </div>
  </div>
  <div class="clear">
