...
<div class="solidBox dataEntry">
  <div class="fieldContainer horizontalFlow">
    ...
    <div>
      <div>
        ...
        <%:Html.TextBoxFor(couponRecord => couponRecord.TicketOrFimIssuingAirline, new { @class = "autocComplete populated", maxlength = 4, readOnly = true })%>
      </div>
      <div>
        <label for="couponNumber">
          <span>*</span> Coupon Number:</label>
        <%:Html.TextBoxFor(couponRecord => couponRecord.TicketOrFimCouponNumber, new { @class = "digits integer populated", readOnly = true })%>
      </div>
      <div>
        <label for="TicketDocOrFimNumber">
          <span>*</span> Ticket/FIM Number:</label>
        <%:Html.TextBoxFor(couponRecord => couponRecord.TicketDocOrFimNumber, new { max = 99999999999, min = 0, @class = "digits integer", @maxLength = 11, readOnly = true })%>
      </div>
      <div>
        <label for="checkDigit">
          <span>*</span> Check Digit:</label>
        <%:Html.TextBoxFor(couponRecord => couponRecord.CheckDigit, new { @class = "checkDigit", @maxLength = 1, readOnly = true })%>
      </div>
    </div>
  ...
  <div>
    ...
    <div>
      ...
      <%:Html.TextBoxFor(couponRecord => couponRecord.IscAmount, new { @class = "amountTextfield amount", min = -99999999999.99, max = 99999999999.99, watermark = ControlIdConstants.NegativeAmount, readOnly = true })%>&nbsp;Amount
    </div>
    <div>
      <label for="taxAmount">
        Tax Amount:</label>
      ...
      <%:ScriptHelper.GenerateDialogueHtml("Tax Breakdown", "Prime Billing Tax Capture", "divTaxBreakdown", 500, 500)%>
    </div>
    <div>
  ...
  <div>
    <div>
      ...
      <%:Html.TextBoxFor(couponRecord => couponRecord.OtherCommissionAmount, new { @class = "amountTextfield amount", min = -99999999999.99, max = 99999999999.99, watermark = ControlIdConstants.NegativeAmount, readOnly = true })%>&nbsp;Amount
    </div>
    <div>
      ...
      <%:Html.TextBoxFor(couponRecord => couponRecord.UatpAmount, new { @class = "amountTextfield amount", min = -99999999999.99, max = 99999999999.99, watermark = ControlIdConstants.NegativeAmount, readOnly = true })%>&nbsp;Amount
    </div>
    <div>
      <label for="vatAmount">
        VAT Amount:</label>
  ...
  <div>
    ...
    <div>
      <label for="settlementAuthorizationCode">
        ESAC:</label>
      <%:Html.TextBoxFor(couponRecord => couponRecord.SettlementAuthorizationCode, new { @maxLength = 14, readOnly = true })%>
    </div>
  </div>
  <div class="fieldSeparator">
  </div>
  <div id="divFlightDetails">
    ...
    <div>
      <label for="flightDate">
        Flight Date:</label>
      <%: Html.TextBox(ControlIdConstants.FlightDate, (Model.FlightDate.HasValue == true ? Model.FlightDate.Value.ToString(FormatConstants.DateFormat) : string.Empty), new {})%>
