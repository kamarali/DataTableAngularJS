...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  SIS :: Passenger :: <%: ViewData[ViewDataConstants.BillingType].ToString() %> :: Rejection Memo Coupon
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div class="buttonContainer">
    <% if (Model.Invoice.InvoiceStatusId >= Convert.ToInt32(InvoiceStatusType.ReadyForBilling) && Model.Invoice.InvoiceStatusId < Convert.ToInt32(InvoiceStatusType.ReadyForSubmission))
       {%>
    <input class="secondaryButton" type="button" value="Back" onclick="javascript:location.href = '<%: Url.Action("RMView", "FormXFPayables", new { invoiceId = Model.InvoiceId.Value(), transactionId = Model.Id.Value() } ) %>'" />
    <%}
       else
       {%>
    <% using (Html.BeginForm("RMCouponCreate", "FormXF", FormMethod.Get))
       { %>
    <input type="submit" value="Add" class="primaryButton" />
    <%} %>
    <input class="secondaryButton" type="button" value="Back" onclick="javascript:location.href = '<%:Url.RouteUrl("transactions", new {action = "RMEdit", controller = "FormXF", invoiceId = Model.InvoiceId.Value(), transactionId = Model.Id.Value() } ) %>'" />
    <%}%>
  </div>
</asp:Content>
