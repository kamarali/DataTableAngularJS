...
<asp:Content ID="Content2" ContentPlaceHolderID="Script" runat="server">
  ...
  <%}%> 
      ...
      InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.CreditMemoRecord.Invoice.BilledMemberId, BillingCategoryType.Pax) %>', '<%: Url.Action("CreditMemoCouponAttachmentDownload","CreditNote") %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
    });
  </script>
  <script type="text/javascript">
...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div class="buttonContainer">
    ...
    <%if (ViewData[ViewDataConstants.PageMode] != null && ViewData[ViewDataConstants.PageMode].ToString() == "View")
      {%>
    <input class="secondaryButton" type="button" value="Back" onclick="javascript:location.href = '<%: Url.Action("CreditMemoView" ,"CreditNote", new {transactionId = Model.CreditMemoRecord.Id.Value(), invoiceId = Model.CreditMemoRecord.InvoiceId.Value()}) %>'" />
    <%}
      else
      { %>
