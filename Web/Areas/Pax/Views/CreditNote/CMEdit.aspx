...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  SIS :: Passenger :: <%: ViewData[ViewDataConstants.BillingType].ToString() %> ::
...
%>
    $("#SourceCodeId").autocomplete({ disabled: true });
      ...
      InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.Invoice.BilledMemberId, BillingCategoryType.Pax) %>', '<%: Url.Action("CreditMemoAttachmentDownload","CreditNote", new {invoiceId = Model.InvoiceId}) %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");      
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div>
    <% using (Html.BeginForm("CreditMemoEdit", "CreditNote", FormMethod.Post, new { id = "creditMemoForm", @class = "validCharacters" }))
       {%>
...
%>
    ...
    <input type="submit" value="Add Credit Memo Coupon" class="secondaryButton" id="btnAdd" onclick="javascript:location.href='<%: Url.Action("CreditMemoCouponCreate", "CreditNote", new { invoiceId = Model.Invoice.Id.Value(), transactionId = Model.Id.Value(), transaction = "CMEdit" })%>'; return false;" />
    <% 
  if (!string.IsNullOrEmpty(SessionUtil.PaxCorrSearchCriteria) || !string.IsNullOrEmpty(SessionUtil.PaxInvoiceSearchCriteria))
  {
%>
