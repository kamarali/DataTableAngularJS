...
<asp:Content ID="titleBlock" ContentPlaceHolderID="TitleContent" runat="server">
    SIS :: Cargo :: <%: ViewData[ViewDataConstants.BillingType].ToString() %> :: <%: ViewData[ViewDataConstants.PageMode] %> Credit Note test
</asp:Content>
...
//    }
   ...
   function formatBillingCodeVatTotalColumn(cellValue, options, rowObject) {
      ...
      var linkHtml = '<a href="#" onclick=showBillingCodeCodeVatDialog("<%: Url.Action("GetBillingCodeVatTotal", "CreditNotePayables", new { area = "Cargo" })%>","' + sourceCodeId + '")>' + "Billing Code VAT" + '</a>';
      // return link
      return linkHtml;
    }  
    ...
    {%>        
    <%:ScriptHelper.GenerateCgoPayablesCNoteGridViewRejectScript(Url, ControlIdConstants.CreditMemoGrid, Url.Action("CMView"), "CM", Model.Id.Value(), Url.Action("InitiateRejection", "CreditNotePayables", new { area = "Cargo" }), Url.Action("InitiateDuplicateRejection", "CreditNotePayables", new { area = "Cargo" }), Model.BillingCode, Model.BillingYear, Model.BillingMonth, Model.BillingPeriod, Model.SettlementMethodId)%>
    <%}
    else
    ...
    <%
      ...
      {
    %>
    <%:ScriptHelper.GenerateGridEditDeleteScript(Url, ControlIdConstants.CreditMemoGrid, Url.RouteUrl("CGOtransactions", new { action = "CMEdit", controller = "CreditNotePayables" }), Url.Action("CreditMemoDelete"))%>
    <%
          }
    %>
</asp:Content>
