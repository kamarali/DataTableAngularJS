...
%>
    $("#SourceCodeId").autocomplete({ disabled: true });
      ...
      InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.Invoice.BilledMemberId, BillingCategoryType.Cgo) %>', '<%: Url.Action("CreditMemoAttachmentDownload","CreditNotePayables", new {invoiceId = Model.InvoiceId}) %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");      
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  else
  {
        ...
        <input type="submit" value="AWB Prepaid Breakdown" class="secondaryButton" id="btnAdd"
            onclick="javascript:location.href='<%: Url.Action("CMAwbPrepaidCreate", "CreditNote", new { invoiceId = Model.Invoice.Id.Value(), transactionId = Model.Id.Value(), transaction = "CMEdit" })%>'; return false;" />
        <input type="submit" value="AWB Charge Collect Breakdown" class="secondaryButton"
            id="btnAddChargeCollectAwb" onclick="javascript:location.href='<%:Url.Action("CMAwbChargeCollectCreate", "CreditNote", new {invoiceId = Model.InvoiceId.Value(), transaction = "CMEdit", transactionId = Model.Id.Value() })%>'; return false;" />
        <% 
           if (!string.IsNullOrEmpty(SessionUtil.CGOCorrSearchCriteria) || !string.IsNullOrEmpty(SessionUtil.CGOInvoiceSearchCriteria))
           {
        %>
