<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<Iata.IS.Model.Cargo.CMAirWayBill>" %>
<%@ Import Namespace="Iata.IS.Web.Util" %>

<h2>
  AWB Prepaid Billing Details</h2>
  <div class="solidBox dataEntry">
    <div class="fieldContainer horizontalFlow">
      <div>
        ...
        <div>
          <label >
            <span class="required">*</span> AWB Serial Number & Check Digit:</label>
      ...
      <div>
        <div>
         ...
         <%:Html.TextBoxFor(awbRecord => awbRecord.CreditedWeightCharge, new { @class = "breakDownDerived amount amt_11_3", watermark = ControlIdConstants.PositiveAmount })%>
        </div>
        <div>
         <label>Valuation Charges:</label>
         <%:Html.TextBoxFor(awbRecord => awbRecord.CreditedValuationCharge, new { @class = "breakDownDerived amount amt_11_3", watermark = ControlIdConstants.PositiveAmount })%>
        </div>
        <div>
         <label>Other Charges:</label>
          <%:Html.TextBoxFor(awbRecord => awbRecord.CreditedOtherCharge, new { id = "OtherChargeAmount", @class = "amountTextfield amount amt_11_3", @readOnly = true })%>
        <%:ScriptHelper.GenerateDialogueHtml("OC Breakdown", "Capture AWB Other Charge Breakdown", "divOtherCharge", 500, 900)%>
        </div>
        <div>
         <label>ISC %:</label>
         <%:Html.TextBoxFor(awbRecord => awbRecord.CreditedIscPercentage, new { @class = "percentageTextfield percent amt_5_3", watermark = ControlIdConstants.NegativePercentage })%>
        </div>
        <div>
         <label>ISC Amount:</label>
         <%:Html.TextBoxFor(awbRecord => awbRecord.CreditedIscAmount, new { @class = "breakDownDerived amount amt_11_3", @readOnly = true, watermark = ControlIdConstants.NegativeAmount })%>
        </div>
      </div>
      <div>
        <div>
         <label>VAT Amount: </label>
         <%:Html.TextBoxFor(awbRecord => awbRecord.CreditedVatAmount, new { id = "VatAmount", @class = "amountTextfield amount amt_11_3", @readOnly = true })%>
        <%:ScriptHelper.GenerateDialogueHtml("VAT Breakdown", "Credit Memo AWB VAT Capture", "divVatBreakdown", 500, 900, "vatBreakdown")%>
        </div>
        <div>
         <label><span class="required">*</span> Total Amount Credited:</label>
         <%:Html.TextBoxFor(awbRecord => awbRecord.TotalAmountCredited, new { @readOnly = true, @class = "amount neg_amt_11_3", watermark = ControlIdConstants.NegativeAmount })%>
        </div>
      </div>
      <div class="fieldSeparator">
      </div>
      <div>
        ...
        <div>
          <label for="proratePercentage">
            Prorate %:</label>
          <%:Html.TextBoxFor(awbRecord => awbRecord.ProratePercentage, new { @class = "integer populated", @maxlength = 2, min = 1, max = 99 })%>
        </div>
      ...
      <div id="divReasonCode">
        <div>
          <label for="reasonCode">
            Reason Code:</label>
          <%:Html.TextBoxFor(awbRecord => awbRecord.ReasonCode, new { @class = "autocComplete populated", maxLength = 2 })%>
        </div>
        <div>
