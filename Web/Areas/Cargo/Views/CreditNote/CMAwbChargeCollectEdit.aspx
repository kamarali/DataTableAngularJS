...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  SIS :: Cargo :: <%: ViewData[ViewDataConstants.BillingType].ToString() %> ::CM AWB Prepaid Invoice :: Create CM AWB Charge Collect Billing
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <h1><%: ViewData[ViewDataConstants.PageMode] %> AWB Charge Collect Billing</h1>
  ...
  <div>
    ...
    <input type="submit" value="Save and Add New" class="primaryButton ignoredirty" id="btnSaveAndAddNew"
      onclick="javascript:return SaveCMAwbRecord('<%: Url.Action("CMAwbChargeCollectEdit","CreditNote",new {transactionId = Model.CreditMemoRecord.Id.Value(),  invoiceId= Model.CreditMemoRecord.Invoice.Id.Value(), transaction="CMEdit", couponId = Model.Id }) %>')" />
    <input type="submit" value="Save and Duplicate" class="primaryButton ignoredirty"
      id="btnSaveAndDuplicate" onclick="javascript:return SaveCMAwbRecord('<%: Url.Action("CMChargeCollectAwbClone","CreditNote",new { transactionId = Model.CreditMemoRecord.Id.Value(),  invoiceId= Model.CreditMemoRecord.Invoice.Id.Value(), transaction="CMEdit" , couponId = Model.Id}) %>')" />
    <input type="submit" value="Save and Back to Overview" class="primaryButton ignoredirty"
      id="SaveAndBackToOverview" onclick="javascript:return SaveCMAwbRecord('<%: Url.Action("CMChargeCollectAwbEditAndReturn","CreditNote",new {transactionId = Model.CreditMemoRecord.Id.Value(),  invoiceId= Model.CreditMemoRecord.Invoice.Id.Value(), transaction="CMEdit" , couponId = Model.Id}) %>')" />
   ...
   <%if (ViewData[ViewDataConstants.PageMode] != null && ViewData[ViewDataConstants.PageMode].ToString() == "View")
      {%>
        <%:Html.LinkButton("Back to View Invoice", Url.RouteUrl("CGOtransactions", new { action = "CMView", invoiceId = Model.CreditMemoRecord.InvoiceId.Value(), transactionId = Model.CreditMemoRecord.Id.Value() }))%>
    <%}
    else
    {%>
