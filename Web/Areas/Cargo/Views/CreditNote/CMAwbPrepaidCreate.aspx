...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  ...
  ::CM AWB Prepaid Invoice :: Add CM AWB Prepaid Billing
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <h1>
    Add AWB Prepaid Billing</h1>
  <div>
    <%
      Html.RenderPartial(Url.Content("ReadOnlyCreditNoteHeaderControl"), Model.CreditMemoRecord.Invoice);%>
  </div>
  ...
  <div>
    ...
    <input type="submit" value="Save and Add New" class="primaryButton ignoredirty" id="btnSaveAndAddNew"
      onclick="javascript:return SaveCMAwbRecord('<%: Url.Action("CMAwbPrepaidCreate","CreditNote",new { transactionId = Model.CreditMemoRecord.Id.Value(),  invoiceId= Model.CreditMemoRecord.Invoice.Id.Value(), transaction="CMEdit" }) %>')" />
    <input type="submit" value="Save and Duplicate" class="primaryButton ignoredirty"
      id="btnSaveAndDuplicate" onclick="javascript:return SaveCMAwbRecord('<%: Url.Action("CMAwbPrepaidCreateAndDuplicate","CreditNote",new { transactionId = Model.CreditMemoRecord.Id.Value(),  invoiceId= Model.CreditMemoRecord.Invoice.Id.Value(), transaction="CMEdit" }) %>')" />
    <input type="submit" value="Save and Back to Overview" class="primaryButton ignoredirty"
      id="SaveAndBackToOverview" onclick="javascript:return SaveCMAwbRecord('<%: Url.Action("CMAwbPrepaidCreateAndReturn","CreditNote",new { transactionId = Model.CreditMemoRecord.Id.Value(),  invoiceId= Model.CreditMemoRecord.Invoice.Id.Value(), transaction="CMEdit" }) %>')" />
