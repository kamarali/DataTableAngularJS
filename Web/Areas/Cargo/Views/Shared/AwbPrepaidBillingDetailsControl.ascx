...
<div class="solidBox dataEntry">
  <div class="fieldContainer horizontalFlow">
    ...
    <div>
      <%--<div>
        <label for="AWBGrossValueOrApplicableLocalFare">
          AWB Gross Value:</label>
        <%:Html.TextBoxFor(awbRecord => awbRecord.AWBGrossValueOrApplicableLocalFare, new { min = 0, max = 99999999999.9, watermark = ControlIdConstants.PositiveAmount, @class = "amount" })%>
      </div>--%>
      <div>
      ...
      <div>
        ...
        <%:Html.TextBoxFor(awbRecord => awbRecord.ValuationCharges, new { @class = "amount amt_11_3", watermark = ControlIdConstants.PositiveAmount })%>
      </div>
      <div>
        <label for="IscPercent">
          ISC:</label>
        <%:Html.TextBoxFor(awbRecord => awbRecord.IscPer, new { @class = "percentageTextfield percent", min = -99.999, max = 99.999, watermark = ControlIdConstants.NegativePercentage })%>&nbsp;%&nbsp;
        <%:Html.TextBoxFor(awbRecord => awbRecord.IscAmount, new { @class = "amountTextfield  amount amt_11_3", @readOnly = true, watermark = ControlIdConstants.NegativeAmount })%>&nbsp;Amount
      </div>
      <%--<div>
        <label for="taxAmount">
          Tax Amount:</label>
        <%:Html.TextBoxFor(awbRecord => awbRecord.TaxAmount, new { @class = "amountTextfield amount", @readOnly = true, min = -99999999999.9, max = 99999999999.9 })%>
        <%:ScriptHelper.GenerateDialogueHtml("Tax Breakdown", "AWB Prepaid Tax Capture", "divTaxBreakdown", 500, 500)%>
      </div>--%>
      <div>
      ...
      <%--<div>
        <label for="Weight Charges">
          Weight Charges:</label>
        <%-- <%:Html.TextBoxFor(awbRecord => awbRecord.OtherCommissionPercent, new { @class = "percentageTextfield percent", max = 99.999, min = -99.999, watermark = ControlIdConstants.NegativePercentage })%>&nbsp;%&nbsp;
      <%:Html.TextBoxFor(awbRecord => awbRecord.WeightCharges, new { @class = "amountTextfield amount", min = -99999999999.99, max = 99999999999.99, watermark = ControlIdConstants.NegativeAmount })%>&nbsp;Amount
    </div>--%>
      <div>
        <label for="vatAmount">
          VAT Amount:</label>
    ...
    <div>
      ...
      <%--<div> This field detail is not mention in prepaid docs thats why we are not using in Prepaid
