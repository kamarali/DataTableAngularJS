...
<%@ Import Namespace="Iata.IS.Business.Security.Permissions.Cargo.DownloadCorrespondences" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <%
       ...
       <div>
        <%
            Html.RenderPartial("CargoCorrespondenceControl", Model);%>
        <div class="buttonContainer">
           ...
           <%--<% if (Html.IsAuthorized(DownloadCorrespondences.PDF))
            {%>--%>
            <input class="primaryButton ignoredirty" onclick="javascript:window.open('<%:Url.Action("DownloadCorrespondence", "Correspondence", new { invoiceId = Model.InvoiceId, transactionId = Model.Id })%>');"
                type="button" value="Download PDF" />
            <%--<%}%>--%>
            <input class="primaryButton ignoredirty" id="attachmentBreakdown" onclick="return openAttachment();"
                type="button" value="Attachments" />
          ...
          <%--CMP527: show close correspondence button on pre define conditions.--%>
            <%if (Convert.ToBoolean(ViewData[ViewDataConstants.CorrespondenceCanClose]))
              {%>
                   ...
                   <input class="primaryButton ignoredirty" onclick="return CheckIfBMExistsThenCloseCorrespondence('<%: Model.CorrespondenceNumber%>', '<%:Model.Id%>', '<%: Model.AuthorityToBill%>', '<%: Url.Action("CheckIfBMExistsOnReply", "Correspondence", new { area = "cargo" }) %>', '<%:Convert.ToInt16(ViewData[ViewDataConstants.CorrespondeneClosedScenario])%>')" type="button" value="Close Correspondence" />
                <%
            }%>
