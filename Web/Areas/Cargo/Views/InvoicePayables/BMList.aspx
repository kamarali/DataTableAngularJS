...
<script src="<%:Url.Content("~/Scripts/Cargo/Billinghistory.js")%>" type="text/javascript"></script>
   <%
    ...
    {%>
  <%:ScriptHelper.GenerateCgoPayablesGridViewRejectScript(Url, ControlIdConstants.TransactionGridId, Url.Action("BMView"), "BM", Model.Id.Value(), Url.Action("InitiateRejection", "InvoicePayables", new { area = "Cargo" }), Url.Action("InitiateDuplicateRejection", "InvoicePayables", new { area = "Cargo" }), Model.BillingCode, Model.BillingYear, Model.BillingMonth, Model.BillingPeriod, Model.SettlementMethodId)%>
  <%}
