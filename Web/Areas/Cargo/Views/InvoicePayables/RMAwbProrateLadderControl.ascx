<%--<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<Iata.IS.Model.Cargo.RMAwbProrateLadder>" %>
<h2>
...
<div class="solidBox">
  ...
  <div>
    ...
    <div class="buttonContainer">
        <input class="primaryButton ignoredirty" type="submit" value="Edit" onclick="javascript:return changeAction('<%: Url.Action("RMAwbProrateLadderUpdate","Invoice",new { couponId =Model.ParentId.Value(),  prorateLadderId= Model.Id.Value(), transaction="RMEdit" }) %>')" />
        <input class="primaryButton ignoredirty" type="submit" value="Delete" onclick="javascript:return changeAction('<%: Url.Action("RMAwbProrateLadderDelete","Invoice",new {  couponId =Model.ParentId.Value(), prorateLadderId= Model.Id.Value(), transaction="RMEdit" }) %>')" />
      </div>
...
<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<List<Iata.IS.Model.Cargo.RMAwbProrateLadderDetail>>" %>
...
<div>
  <% Html.RenderPartial("RMAwbProrateLadderDetailsControl", new RMAwbProrateLadderDetail());%>
</div>
<div class="buttonContainer">
