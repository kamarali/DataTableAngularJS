...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.BillingMemoRecord.Invoice.BilledMemberId, BillingCategoryType.Cgo) %>', '<%: Url.Action("BMAwbAttachmentDownload","InvoicePayables") %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");    
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <h1>
    <%:ViewData[ViewDataConstants.PageMode] %> AWB Charge Collect Billing</h1>
  <div>
    <%
  ...
  <div class="buttonContainer">
    <input type="submit" value="Save and Add New" class="primaryButton ignoredirty" id="btnSaveAndAddNew"
      onclick="javascript:return SaveBMAwbRecord('<%: Url.Action("BMAwbChargeCollectEdit","Invoice",new {transactionId = Model.BillingMemoRecord.Id.Value(),  invoiceId= Model.BillingMemoRecord.Invoice.Id.Value(), transaction="BMEdit", couponId = Model.Id }) %>')" />
    <input type="submit" value="Save and Duplicate" class="primaryButton ignoredirty"
      id="btnSaveAndDuplicate" onclick="javascript:return SaveBMAwbRecord('<%: Url.Action("BMChargeCollectAwbClone","Invoice",new { transactionId = Model.BillingMemoRecord.Id.Value(),  invoiceId= Model.BillingMemoRecord.Invoice.Id.Value(), transaction="BMEdit" , couponId = Model.Id}) %>')" />
    <input type="submit" value="Save and Back to Overview" class="primaryButton ignoredirty"
      id="SaveAndBackToOverview" onclick="javascript:return SaveBMAwbRecord('<%: Url.Action("BMChargeCollectAwbEditAndReturn","Invoice",new {transactionId = Model.BillingMemoRecord.Id.Value(),  invoiceId= Model.BillingMemoRecord.Invoice.Id.Value(), transaction="BMEdit" , couponId = Model.Id}) %>')" />
