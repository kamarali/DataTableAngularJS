...
<asp:Content ID="Content3" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div class="buttonContainer">
    ...
    <%
      ...
      {%>
    <input class="secondaryButton" type="button" value="Back to View Invoice" onclick="javascript:location.href = '<%:Url.Action("View","InvoicePayables",new{invoiceId = Model.Id})%>';" />
    <%}%>
  </div>
...
<asp:Content ID="Content2" ContentPlaceHolderID="Script" runat="server">
  ...
  <%
    ...
    {%>
  <%:ScriptHelper.GenerateCgoPayablesGridViewRejectScript(Url, ViewDataConstants.RejectionMemoListGrid, Url.Action("RMView", new { billingType = ViewData[ViewDataConstants.BillingType].ToString() }), "RM", Model.Id.Value(), Url.Action("InitiateRejection", "InvoicePayables", new { area = "Cargo" }), Url.Action("InitiateDuplicateRejection", "InvoicePayables", new { area = "Cargo" }), Model.BillingCode, Model.BillingYear, Model.BillingMonth, Model.BillingPeriod, Model.SettlementMethodId)%>
  <%} %>
