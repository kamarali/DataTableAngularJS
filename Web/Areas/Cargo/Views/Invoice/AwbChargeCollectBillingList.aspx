...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  ...
  :: <%--<%:ViewData[ViewDataConstants.PageMode] %> Non-Sampling--%> Invoice :: Charge Collect AWB Listing
</asp:Content>
<%--<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
 <h1>
    AWB Charge Collect Billing Page (Page is not completed - Work in Progress)</h1>
</asp:Content>--%>
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <h1>
    Charge Collect AWB Billing Records</h1>
  <div>
    <% Html.RenderPartial("ReadOnlyInvoiceHeaderControl", Model); %>
  </div>
  <h2>
    AWB Charge Collect Billing Record List</h2>
  <div>
  ...
  <div class="buttonContainer">
    ...
    <%
      ...
      else
      {
        ...
        {%>
<%--    <input class="primaryButton" type="button" value="Reject" onclick="javascript:InitiateRejForSpecificTrans('<%: ViewDataConstants.PrimeBillingGrid %>', 'PC', '<%: Model.Id %>', '<%:Url.Action("InitiateRejection", "Invoice", new { area = "Pax" })%>','<%:Url.Action("InitiateDuplicateRejection", "Invoice", new { area = "Pax" })%>', <%:Model.BillingCode %>, <%:Model.BillingYear %>,<%:Model.BillingMonth %>,<%:Model.BillingPeriod %>,<%:Model.SettlementMethodId %>);" id ="RejectButton" disabled ="disabled"/>
    <%}%>
    ...
    <%
        ...
        <input class="primaryButton" type="button" value="Reject" onclick="javascript:InitiateRejForSpecificTrans('<%: ViewDataConstants.PrimeBillingGrid %>', 'AWB', '<%: Model.Id %>', '<%:Url.Action("InitiateRejection", "Invoice", new { area = "Cargo" })%>','<%:Url.Action("InitiateDuplicateRejection", "Invoice", new { area = "Cargo" })%>', <%:Model.BillingCode %>, <%:Model.BillingYear %>,<%:Model.BillingMonth %>,<%:Model.BillingPeriod %>,<%:Model.SettlementMethodId %>);" id ="RejectButton" />
    <%}%>
    <input class="secondaryButton" type="button" value="Back to View Invoice" onclick="javascript:location.href = '<%:Url.Action("View","Invoice",new{invoiceId = Model.Id.Value()})%>';" />
    <%
      }%>
  </div>
...
<script src="<%:Url.Content("~/Scripts/Cargo/AwbRecord.js")%>" type="text/javascript"></script>
  ...
  <%:ScriptHelper.GenerateGridViewScript(Url, ViewDataConstants.PrimeBillingGrid, Url.Action("AwbChargeCollectBillingView"))%>
  <%
    }
    else
    {
  %>
  <%:ScriptHelper.GenerateGridEditDeleteScript(Url, ViewDataConstants.PrimeBillingGrid, Url.Action("AwbChargeCollectRecordEdit"), Url.Action("AwbPrepaidRecordDelete"))%>
  <%
    }
  %>
