...
<asp:Content ID="Content1" ContentPlaceHolderID="TitleContent" runat="server">
  ...
  ::BM AWB Prepaid Invoice :: Create BM Prepaid AWB
</asp:Content>

<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  <h1>
    Create AWB Prepaid Billing </h1>
  <div>
    <%
  ...
  <div class="buttonContainer">
    <input type="submit" value="Save and Add New" class="primaryButton ignoredirty" id="btnSaveAndAddNew"
      onclick="javascript:return SaveBMAwbRecord('<%: Url.Action("BMAwbPrepaidCreate","Invoice",new { transactionId = Model.BillingMemoRecord.Id.Value(),  invoiceId= Model.BillingMemoRecord.Invoice.Id.Value(), transaction="BMEdit" }) %>')" />
    <input type="submit" value="Save and Duplicate" class="primaryButton ignoredirty"
      id="btnSaveAndDuplicate" onclick="javascript:return SaveBMAwbRecord('<%: Url.Action("BMAwbPrepaidCreateAndDuplicate","Invoice",new { transactionId = Model.BillingMemoRecord.Id.Value(),  invoiceId= Model.BillingMemoRecord.Invoice.Id.Value(), transaction="BMEdit" }) %>')" />
    <input type="submit" value="Save and Back to Overview" class="primaryButton ignoredirty"
      id="SaveAndBackToOverview" onclick="javascript:return SaveBMAwbRecord('<%: Url.Action("BMAwbPrepaidCreateAndReturn","Invoice",new { transactionId = Model.BillingMemoRecord.Id.Value(),  invoiceId= Model.BillingMemoRecord.Invoice.Id.Value(), transaction="BMEdit" }) %>')" />
