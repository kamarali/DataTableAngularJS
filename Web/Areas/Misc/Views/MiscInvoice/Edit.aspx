...
<!--SIS_SCR_REPORT_23_jun-2016_2 : Value_Shadowing Change Request[""] to Request.Form[""] -->
  ...
  <%
    ...
    InitiliazeCorrespondenceFields('<%:Model.Id%>', true);
      ...
      '<%:Url.Action("IsRejectionOutsideTimeLimit", "MiscInvoice", new { area = "Misc" })%>', '<%:Url.Action("IsBillingAndBilledAchOrDualMember", "Data", new { area = "" })%>',
    ...
    InitializeAttachmentGrid(<%=new JavaScriptSerializer().Serialize(Model.Attachments)%>, '<%:new FileAttachmentHelper().GetValidFileExtention(Model.BilledMemberId, Model.BillingCategory)%>','<%:Url.Action("DeleteAttachment", "MiscInvoice")%>','<%:Url.Action("InvoiceAttachmentDownload", "MiscInvoice")%>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
...
function RejectInvoice() {
  ...
  while (part_num < selectedLineItemIds.length)
 {
  ...
  part_num++;
      ...
      {
%>CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "p", '<%:Url.Action("RejectInvoice", "MiscInvoice")%>');
     <%
      }
      else if (TempData["Reject"] != null && Convert.ToBoolean(TempData["Reject"])) // from billing history
      {%>
  CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "bh", '<%:Url.Action("RejectInvoice", "MiscInvoice")%>');
  <%
      }%>
  ...
  <%
    ...
    {%>
  <%: ScriptHelper.GenerateGridEditDeleteScript(Url, ControlIdConstants.LineItemGridId, Url.Action("LineItemEdit", "MiscInvoice"),  Url.Action("LineItemDelete", "MiscInvoice"))%>
  <%
    }
...
<%
    ...
    <%if (Model.InvoiceStatusId == (int)InvoiceStatusType.ReadyForSubmission && (Html.IsAuthorized(Iata.IS.Business.Security.Permissions.Misc.Receivables.Invoice.CreateOrEdit) || Html.IsAuthorized(Iata.IS.Business.Security.Permissions.Misc.Receivables.Invoice.View)))
      {%>
