...
<asp:Content ID="Content1" ContentPlaceHolderID="Script" runat="server">
  ...
  <%
    ...
    InitialiseInvoiceHeader('<%:Url.Action("GetMiscUatpBilledMemberLocationList", "Data", new { area = "" })%>', 
      ...
      '<%:Url.Action("GetRejectionStageForSmi", "Data", new { area = "" })%>', '', '<%:Url.Action("IsBillingAndBilledAchOrDualMember", "Data", new { area = "" })%>',
      '<%: Model.BillingCurrencyId %>');
    ...
    InitializeAttachmentGrid(<%= new JavaScriptSerializer().Serialize(Model.Attachments) %>, '<%: new FileAttachmentHelper().GetValidFileExtention(Model.BilledMemberId, Model.BillingCategory) %>', '<%: Url.Action("DeleteAttachment", "MiscCreditNote") %>','<%: Url.Action("InvoiceAttachmentDownload","MiscCreditNote") %>', "<%:Url.Content("~/Content/Images/busy.gif")%>");
  ...
  while (part_num < selectedLineItemIds.length)
 {
  ...
  part_num++;
      ...
      {
%>CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "p", '<%:Url.Action("RejectInvoice", "MiscCreditNote")%>');
     <%
      }
      else if (TempData["Reject"] != null && Convert.ToBoolean(TempData["Reject"])) // from billing history
      {%>
  CallRejectInvoice('<%:Model.Id%>',selectedItemNos, "bh", '<%:Url.Action("RejectInvoice", "MiscCreditNote")%>');
  <%
      }%>
}
  </script>
  ...
  <% if ((string)ViewData[ViewDataConstants.PageMode] != PageMode.View)
     { %>
  <% =ScriptHelper.GenerateGridEditDeleteScript(Url, ControlIdConstants.LineItemGridId, Url.Action("LineItemEdit", "MiscCreditNote"), Url.Action("LineItemDelete", "MiscCreditNote"))%>
  <% }
...
<asp:Content ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div>
    <%:Html.Hidden("IsLineItemExistHid", (ViewData[ViewDataConstants.TransactionExists] != null)?ViewData[ViewDataConstants.TransactionExists].ToString().ToLower():string.Empty)%>
...
{%>
    ...
    <input class="secondaryButton" type="button" value="Attachments" id="UploadAttachment" />
     <%-- CMP288: code to create preview button and it's logic--%>
     <%if (Model.InvoiceStatusId == (int)InvoiceStatusType.ReadyForSubmission && (Html.IsAuthorized(Iata.IS.Business.Security.Permissions.Misc.Receivables.CreditNote.CreateOrEdit) || Html.IsAuthorized(Iata.IS.Business.Security.Permissions.Misc.Receivables.CreditNote.View)))
      {%>
    <input type="button" value="Preview Invoice" class="secondaryButton" onclick="javascript:window.open('<%:Url.Action("PreviewInvoiceOnScreen", "MiscInvoice", new {invoiceId = Model.Id})%>');" />
    <%}%>
  </div>
  <%} %>
