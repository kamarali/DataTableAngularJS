...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
  ...
  <div class="buttonContainer">
    ...
    <input class="secondaryButton" value="First" onclick = "javascript:location.href='<%: Url.Action("LineItemDetailEdit", "MiscCreditNote", new { invoiceId = Model.LineItemDetail.LineItem.InvoiceId, lineItemId = Model.LineItemDetail.LineItem.Id, lineItemDetailId = Model.LineItemDetail.LineItem.NavigationDetails.FirstId }) %>'" type="button"/>
    <input class="secondaryButton" value="Previous" onclick = "javascript:location.href='<%: Url.Action("LineItemDetailEdit", "MiscCreditNote", new { invoiceId = Model.LineItemDetail.LineItem.InvoiceId, lineItemId = Model.LineItemDetail.LineItem.Id, lineItemDetailId = Model.LineItemDetail.LineItem.NavigationDetails.LastId }) %>'" type="button"/>
    <%} else
    {%>
   ...
   <div class="buttonContainer">
    <input class="primaryButton ignoredirty" type="submit" value="Save And Add" id="Save" onclick="javascript:return changeAction('<%: Url.Action("LineItemDetailCreate","MiscCreditNote") %>')"  />
    <input class="primaryButton ignoredirty" type="submit" value="Save And Duplicate" id="SaveDuplicate" onclick="javascript:return changeAction('<%: Url.Action("LineItemDetailDuplicate","MiscCreditNote") %>')" />
    <input class="primaryButton ignoredirty" type="submit" value="Save And Back" id="SaveReturn" onclick="javascript:return changeAction('<%: Url.Action("LineItemDetailReturn","MiscCreditNote") %>')" />
    <% if (ViewData[ViewDataConstants.PageMode] != null && ViewData[ViewDataConstants.PageMode].ToString() == PageMode.View)
       { %>
      <%: Html.LinkButton("Back", Url.Action("LineItemView", "MiscPayCreditNote"))%>
    <%} else {%>
