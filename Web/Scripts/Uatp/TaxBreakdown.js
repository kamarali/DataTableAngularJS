...
function InitializeTaxGrid(taxData) {
  ...
  $validateTax = $("#formTaxDetails").validate({
    rules: {
      ...
      TaxDescription: { maxlength: 50 },
      Percentage: { min: -999.99, max: 999.99 }
    },
    messages: {
      SubType: {
  ...
  $taxList.jqGrid({
    ...
    colModel: [
                ...
                { name: taxFields.TaxPercent.Id, index: taxFields.TaxPercent.Id, sortable: false, align: 'right', formatter: percentageFormatter },
...
function addTax() {
  ...
  $("#TaxDetails").children("div").children("div").children("input").each(function (i) {
      ...
      if (currentElem.attr("name") != taxFields.Description.Id) {
