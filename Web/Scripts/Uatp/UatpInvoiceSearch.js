...
$(document).ready(function () {
    ...
    $("#MiscInvoiceSearchForm").validate({
        ...
        messages: {
            ...
            AssociatedLocation: "Billed To Location ID required"
        }
    });
...
function BindSelectedLocation() {
    ...
    $("#AssociatedLocation option:selected").each(function () {
        selectedLocationIds = selectedLocationIds + ",'" + $(this).text().toUpperCase() + "'";
    });
...
function resetUatpInvoiceSearchForm(formId) {
    ResetForm(formId);
    $("#BillingCode").val("-1");
    $("#InvoiceStatus").val("-1");
...
function ResetSearch(formId) {
    resetUatpInvoiceSearchForm(formId);
    $.ajax({
        type: "POST",
        url: clearSearchUrl,
        dataType: "json",
