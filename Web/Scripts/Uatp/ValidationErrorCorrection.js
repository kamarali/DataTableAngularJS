...
function DisplayDetails(id) {
  ...
  $.ajax({
    ...
    data: { exceptionCode: exceptionCode, billingCategoryId: "4" },
    dataType: "json",
    success: function (result) {
      if (result) {
        if (result == 1) {
...
function updateCorrctLinkingbuttonclick() {
  if ($('#rmpopup').is(":visible") == true) {
    if ($('#YourInvoiceNo').val() == null || $('#YourInvoiceNo').val() == "") {
      ...
      alert("Settlement Period field is mandatory");
      return;
    }
    ...
    if ($('#CorrespondenceRefNo').val() == null || $('#CorrespondenceRefNo').val() == "") {
      alert("Correspondence Ref Number field is mandatory");
      return;
    }
