...
function prefixAsteriskForChargeCode() {
  var asteriskHtml = '<span style = "color:red">* </span>';
  $("#categoryCodeLabel").html(asteriskHtml + $("#categoryCodeLabel").html());
}

function InitializeVatGrid(vatData) {
  prefixAsteriskForChargeCode();
  ...
  $validateVat = $("#formVatDetails").validate({
    rules: {
      VATDescription: {
        maxlength: 150 //CMP464: change VAT Description Length from 50 to 150
      },
    ...
    messages: {
      VATCategoryCode: { required: "VAT Category required." },
