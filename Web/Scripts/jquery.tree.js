	...
	$.tree = {
		...
		defaults	: {
			...
			rules	: {
				...
				multitree: "TreeChoosePermission,OptionalTreeViewcontrol", // all, none, array of tree IDs to accept from
			...
			refresh : function (obj) {
				...
				_datastore.load(this.callback("beforedata",[false,this]),this,this.settings.data.opts,function(data) {
					...
					_datastore.parse(data,_this,_this.settings.data.opts,function(str) {
						...
						_this.container.find("li:last-child").addClass("last").end().find("li:has(ul)").not(".closed").addClass("open");
