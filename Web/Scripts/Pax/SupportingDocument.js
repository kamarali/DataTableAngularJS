...
function loadAttachment(url, transacId, invId, transacType, isFormCAttachmentAllowed) {
  ...
  // If sample digit announced for the concerning provisional billing period then the attachment grid displayed in view mode.
  var divHeight = 430;
  if (isFormCAttachmentAllowed > 0) {
    divHeight = 320;
    $("#ajaxUploadForm").hide();
    $("#AttachmentGrid").jqGrid("hideCol", "Id");
  }
  $('#divAttachment').dialog({ closeOnEscape: false, title: 'Add/Remove Attachments', height: divHeight, width: 870, modal: true, resizable: false });
...
function deleteAttachment(methodName, value) {
  ...
  if (confirm("Are you sure you want to delete this record?")) {
    ...
    $.ajax({
      type: "POST",
      url: methodName,
      data: { attachmentId: value, transactionType: recordType, invoiceId: invoiceId },
      success: function (result) {
        $('#errorContainer').hide();
